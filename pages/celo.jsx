// components/CeloDetails.js

import React, { useState, useEffect } from 'react';
import axios from 'axios';

const CelosScan = ({ address }) => {
  ...
  useEffect(() => {
    const fetchAddressDetails = async () => {
      try {
        ...
        console.error('Error fetching Celo details:', error);
        setLoading(false);
      }
    };
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 bg-white shadow-md rounded-md">
      <h2 className="text-2xl font-bold mb-4">Celo Details for {address}</h2>
      {loading ? (
        <p>Loading...</p>
      ) : (
        <div>
