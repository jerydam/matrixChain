// components/PolygonDetails.js

import React, { useState, useEffect } from 'react';
import axios from 'axios';

const PolygonScan = ({ address }) => {
    ...
    const fetchAddressDetails = async () => {
      try {
        ...
        const transactionHistoryResponse = await axios.get(
          `https://api.polygonscan.com/api?module=account&action=txlist&address=${address}&startblock=0&endblock=99999999&sort=desc&apikey=${API_KEY}`
        );
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 bg-white shadow-md rounded-md">
      <h2 className="text-2xl font-bold mb-4">Polygon Details for {address}</h2>
      {loading ? (
        <p>Loading...</p>
      ) : (
        <div>
