// components/BscDetails.js

import React, { useState, useEffect } from 'react';
import axios from 'axios';

const Binance = ({ address }) => {
    ...
    const fetchAddressDetails = async () => {
      try {
        ...
        const addressDetailsResponse = await axios.get(
          `https://api.bscscan.com/api?module=account&action=balance&address=${address}&tag=latest&apikey=${API_KEY}`
        );

        // Fetch transaction history
        const transactionHistoryResponse = await axios.get(
          `https://api.bscscan.com/api?module=account&action=txlist&address=${address}&startblock=0&endblock=99999999&sort=desc&apikey=${API_KEY}`
        );
        ...
        console.error('Error fetching BSC details:', error);
        setLoading(false);
      }
    };
